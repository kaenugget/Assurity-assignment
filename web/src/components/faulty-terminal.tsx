...
export function FaultyTerminal({
  ...
  useEffect(() => {
    ...
    try {
      renderer = new Renderer({ dpr, alpha: true, premultipliedAlpha: true });
      ...
      const update = (timeMs: number) => {
        ...
        if (!pause) {
          const elapsed = (timeMs * 0.001 + timeOffsetRef.current) * timeScale;
        ...
        if (pageLoadAnimation && loadAnimationStartRef.current > 0) {
          ...
          const animationElapsed = timeMs - loadAnimationStartRef.current;
        ...
        if (mouseReact) {
          ...
          const uniformMouse = program.uniforms.uMouse.value as Float32Array;
          uniformMouse[0] = smoothMouse.x;
          uniformMouse[1] = smoothMouse.y;
        }
  ...
  return <div ref={containerRef} className={cn("faulty-terminal-container", className)} style={style} {...rest} />;
}
